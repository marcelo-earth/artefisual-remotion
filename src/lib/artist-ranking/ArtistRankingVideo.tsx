...
const IntroCard = ({ artistName }: { artistName: string }) => {
            ...
            >
              <span
                style={{
                  fontSize: "3.4rem",
                  fontWeight: 800,
                  color: isTop ? "#1DB954" : "rgba(255,255,255,0.45)",
                  minWidth: "56px",
                  textAlign: "right",
              ...
              <span
                style={{
                  fontSize: "3.4rem",
                  color: isTop ? "white" : "rgba(255,255,255,0.9)",
...
export const ArtistRankingVideo = ({ artistName, songs, previewUrl, blurredBackground }: Props) => {
  return (
    <AbsoluteFill
      style={{
        ...
        backgroundImage: blurredBackground
          ? undefined
          : "radial-gradient(ellipse at 50% 0%, rgba(29,185,84,0.18) 0%, transparent 60%), radial-gradient(ellipse at 80% 100%, rgba(100,60,200,0.12) 0%, transparent 50%)",
    ...
    >
      ...
      {blurredBackground ? (
        <>
          ...
          <AbsoluteFill
            style={{
              background:
                "linear-gradient(180deg, rgba(0,0,0,0.55) 0%, rgba(0,0,0,0.4) 35%, rgba(0,0,0,0.7) 100%)",
            }}
          />
        </>
      ) : null}
      ...
      <Sequence durationInFrames={INTRO_FRAMES}>
        <IntroCard artistName={artistName} />
      </Sequence>

      <Sequence from={INTRO_FRAMES}>
        <RankingReveal artistName={artistName} songs={songs} hasAudio={Boolean(previewUrl)} />
      </Sequence>
    </AbsoluteFill>
  );
};
